...
export default {
  async fetch(req, env) {
    ...
    if (url.pathname === '/stats' && req.method === 'GET') {
      ...
      const daily = await env.DB.prepare('SELECT day,uv,visits FROM daily ORDER BY day DESC LIMIT 30').all();
    ...
    if (url.pathname === '/duration' && req.method === 'POST') {
      ...
      if (!vid || !Number.isFinite(ms)) return new Response('bad', { status: 400 });
      const ip = req.headers.get('CF-Connecting-IP') || req.headers.get('x-forwarded-for') || '0.0.0.0';
      const dedupKey = await sha256(vid + '|' + ip);
